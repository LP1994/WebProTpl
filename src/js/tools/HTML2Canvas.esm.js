...
var lookup = typeof Uint8Array === 'undefined'
             ? []
             : new Uint8Array( 256 );
for(
    var i = 0;
...
var decode = function ( base64 ){
    ...
    var bytes = Array.isArray( buffer )
                ? buffer
...
var codePointsToCharacterClasses = function ( codePoints, lineBreak ){
    ...
    codePoints.forEach( function ( codePoint, index ){
        ...
        indicies.push( index );
        if( classType === CJ ){
            return types.push( lineBreak === 'strict'
                               ? NS
                               : ID );
        }
        if( classType === SA ){
...
var _lineBreakAtIndex = function ( codePoints, classTypes, indicies, index, forbiddenBreaks ){
    ...
    var before = beforeIndex >= 0
                 ? classTypes[ beforeIndex ]
                 : 0;
...
var stringToNumber = function ( codePoints ){
    ...
    var frac = fracd
               ? parseInt( fromCodePoint.apply( void 0, fraction ), 10 )
               : 0;
...
var getAbsoluteValueForTuple = function ( tuple, width, height ){
    ...
    return [
        ...
        getAbsoluteValue( typeof y !== 'undefined'
                          ? y
                          : x, height )
    ];
};
...
var hsl = function ( args ){
    ...
    var hue = tokens[ 0 ],
        ...
        lightness = tokens[ 2 ],
        alpha = tokens[ 3 ];
    var h = ( hue.type === TokenType.NUMBER_TOKEN
              ? deg( hue.number )
              : angle.parse( hue ) ) / ( Math.PI * 2 );
    var s = isLengthPercentage( saturation )
            ? saturation.number / 100
            : 0;
    var l = isLengthPercentage( lightness )
            ? lightness.number / 100
            : 0;
    var a = typeof alpha !== 'undefined' && isLengthPercentage( alpha )
...
var calculateGradientDirection = function ( angle, width, height ){
    var radian = typeof angle === 'number'
                 ? angle
    ...
    parse: function ( tokens ){
        ...
        for(
            ...
            if( counter.type === TokenType.IDENT_TOKEN ){
                var increment = next && isNumberToken( next )
                                ? next.number
                                : 1;
                increments.push( {
    ...
    parse: function ( tokens ){
        ...
        for(
            ...
            if( isIdentToken( counter ) && counter.value !== 'none' ){
                var reset = next && isNumberToken( next )
                            ? next.number
                            : 0;
                resets.push( {
                    counter: counter.value,
...
var CSSParsedDeclaration = /** @class */ ( function (){
    function CSSParsedDeclaration( declaration ){
        ...
        this.overflowY = overflowTuple[ overflowTuple.length > 1
                                        ? 1
                                        : 0 ];
...
var parse = function ( descriptor, style ){
    ...
    case PropertyDescriptorParsingType.IDENT_VALUE:
        ...
        return descriptor.parse( isIdentToken( token )
                                 ? token.value
                                 : descriptor.initialValue );
...
var breakWords = function ( str, styles ){
    var breaker = LineBreaker( str, {
        ...
        wordBreak: styles.overflowWrap === OVERFLOW_WRAP.BREAK_WORD
                   ? 'break-word'
                   : styles.wordBreak
    } );
    var words = [];
...
var BoundCurves = /** @class */ ( function (){
    function BoundCurves( element ){
        ...
        this.topRightPaddingBox =
            trh > 0 || trv > 0
            ? getCurvePoints( bounds.left + Math.min( topWidth, bounds.width + borderLeftWidth ), bounds.top + borderTopWidth, topWidth > bounds.width + borderLeftWidth
                                                                                                                               ? 0
                                                                                                                               : trh - borderLeftWidth, trv - borderTopWidth, CORNER.TOP_RIGHT )
...
var parseStackTree = function ( parent, stackingContext, realStackingContext, listItems ){
    parent.container.elements.forEach( function ( child ){
        ...
        if( treatAsRealStackingContext || createsStackingContext ){
            ...
            parseStackTree( paintContainer, stack, treatAsRealStackingContext
                                                   ? stack
...
var processListItems = function ( owner, elements ){
    var numbering = owner instanceof OLElementContainer
                    ? owner.start
                    : 1;
    var reversed = owner instanceof OLElementContainer
                   ? owner.reversed
                   : false;
    for(
        var i = 0;
        ...
        return __generator( this, function ( _b ){
            ...
            case 0:
                ...
                _a = isBodyElement( element ) || isHTMLElement( element )
                     ...
                     : parseBounds( element ), width = _a.width, height = _a.height, left = _a.left, top = _a.top;
                defaultResourceOptions = {
                    allowTaint: false,
                    imageTimeout: 15000,
                    proxy: undefined,
