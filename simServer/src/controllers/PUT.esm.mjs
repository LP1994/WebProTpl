...
async function PUTContr( server, request, response ){
    const {
        pathNameStr,
    } = URLTool( request.url );
        ...
        fields = await new Promise( ( resolve = () => {
            ...
            CreateFormidable()
                .parse( request, ( err, fields, files ) => {
                    if( err ){
                        console.log( '------>formidable PUTContr err Start<------' );
                        console.error( err );
                        console.log( '------>formidable PUTContr err End<------' );
